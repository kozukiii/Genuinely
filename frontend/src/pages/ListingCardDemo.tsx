...
const demoListing: Listing = {
  id: "listingcard-demo",
  source: "marketplace",
  title: "Callaway Ai Smoke Max Driver",
  price: 429,
  currency: "USD",
  condition: "Like New",
  url: "https://www.facebook.com/marketplace/",
  images: [
    "https://placehold.co/900x900/0b1020/0b1020",
    "https://placehold.co/900x900/101826/101826",
    "https://placehold.co/900x900/162033/162033",
  ],
  location: "Chicago, IL",
  description: "Selling my Callaway Ai Smoke Max driver, 10.5° loft with the stock Aldila Ascent shaft. Only used for one season — no bag rash, face is clean. Grip is pretty worn, basically falling apart at this point, would need replacing. Picked up a new setup so this has to go. Comes with headcover and torque wrench. Cash or Venmo, local pickup preferred but can ship at buyer's expense.",
};
...
const SUMMARY_TEXT =
  "This listing is priced in the fair range but factor in regripping costs before committing — the seller notes the grip is heavily worn and falling apart, which means an additional $10–20 expense. " +
  "The face and shaft appear clean with no bag rash reported. " +
  "Headcover and torque wrench are included, which is a plus over comparable listings. " +
  "Local pickup preferred, shipping available at buyer's expense — inspect in person if possible given the grip condition.";

// ─── Price graph config ─────────────────────────────────────────────────────

const PRICE_LOW  = 350;
const PRICE_HIGH = 450;
const PRICE_MID  = (PRICE_LOW + PRICE_HIGH) / 2;
...
function priceFillPct(price: number) {
  return Math.min(Math.max(((price - BAR_MIN) / BAR_RANGE) * 100, 0), 100);
}

// ─── Price badge ─────────────────────────────────────────────────────────────

type PriceBadge = { label: string; color: string; bg: string };

function getPriceBadge(price: number): PriceBadge {
  if (price < PRICE_LOW * 0.5)  return { label: "RISKY PRICE", color: "#ef4444", bg: "rgba(239,68,68,0.13)" };
  if (price < PRICE_LOW)        return { label: "GREAT PRICE", color: "#a855f7", bg: "rgba(168,85,247,0.13)" };
  if (price <= PRICE_MID)       return { label: "GOOD PRICE",  color: "#22c55e", bg: "rgba(34,197,94,0.13)" };
  if (price <= PRICE_HIGH)      return { label: "FAIR PRICE",  color: "#facc15", bg: "rgba(250,204,21,0.13)" };
  return                               { label: "HIGH PRICE",  color: "#ef4444", bg: "rgba(239,68,68,0.13)" };
}
...
export default function ListingCardDemo() {
  ...
  const activePrice = priceOverride ?? demoListing.price!;
  const fillPct     = priceFillPct(activePrice);
  ...
  return (
    <div className="listing-page">
      <div className="listing-card-block" style={{ flexDirection: "column" }}>
        <div style={{ display: "flex", gap: "2.4rem" }}>
          ...
          <div className="page-info" style={{ position: "relative" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <span style={{ fontSize: "0.7rem", color: "rgba(255,255,255,0.3)", userSelect: "none" }}>
            ...
            <div className="info-ring-row" style={{ justifyContent: "flex-start", gap: "32px" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: "56px" }}>
                ...
                {analysisPhase !== "idle" && (
                  ...
                  >
                    {(() => {
                      const badge = getPriceBadge(activePrice);
                      const done  = analysisPhase === "done";
                      return (
                        <div
                          className="demo-score-bar-title demo-fade-in"
                          style={done ? { color: badge.color, background: badge.bg, border: `1px solid ${badge.color}50` } : undefined}
                        >
                          {done && badge.label === "GREAT PRICE" && <StarSparkles />}
                          {done ? badge.label : "Price Data"}
                        </div>
                      );
                    })()}
            ...
            {/* Analyze button + marketplace link */}
            <div style={{ position: "absolute", left: 0, right: 0, top: "calc(22rem - 92px)", display: "flex", flexDirection: "column", gap: "8px" }}>
              {analysisScore == null && (
        ...
        {analysisPhase === "done" && (
          <div className="demo-fade-in" style={{ marginTop: "0.6rem" }}>
          <p style={{ margin: "0 0 8px", fontSize: "0.68rem", fontWeight: 600, letterSpacing: "0.06em", color: "rgba(255,255,255,0.3)", textTransform: "uppercase" }}>At a glance</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            {[
              { label: "+ Headcover included",    positive: true  },
              { label: "+ Torque wrench included", positive: true  },
              { label: "+ Clean face · no bag rash", positive: true  },
              { label: "− Needs regripping",      positive: false },
              { label: "− Shipping at buyer's expense", positive: false },
            ].map(({ label, positive }) => (
              <span key={label} style={{
                ...
                padding: "4px 12px",
                borderRadius: "999px",
                color:       positive ? "#22c55e" : "#ff5555",
                background:  positive ? "rgba(34,197,94,0.1)" : "rgba(255,85,85,0.1)",
                border:      `1px solid ${positive ? "rgba(34,197,94,0.3)" : "rgba(255,85,85,0.3)"}`,
                whiteSpace: "nowrap",
              }}>
                {label}
              </span>
        ...
        {/* Dev price toggle — fixed far left */}
        {(() => {
          const tiers: { label: string; price: number }[] = [
            { label: "HIGH PRICE",  price: 499 },
            { label: "FAIR PRICE",  price: 429 },
            { label: "GOOD PRICE",  price: 375 },
            { label: "GREAT PRICE", price: 250 },
            { label: "RISKY PRICE", price: 99  },
          ];
          return (
            <div style={{
              position: "fixed",
              ...
              {tiers.map(({ label, price }) => {
                ...
                const active = (priceOverride ?? demoListing.price!) === price;
                return (
                  <button
                    key={label}
                    type="button"
                    onClick={() => setPriceOverride(active ? null : price)}
                    style={{
                      fontSize: "0.6rem",
                      fontWeight: 700,
                      ...
                      padding: "5px 10px",
                      borderRadius: "999px",
                      border: `1px solid ${badge.color}${active ? "cc" : "55"}`,
                      background: active ? badge.bg : "rgba(0,0,0,0.5)",
                      color: active ? badge.color : `${badge.color}99`,
                      cursor: "pointer",
                  ...
                  >
                    {label}
                    <span style={{ opacity: 0.65, marginLeft: "5px", fontWeight: 500 }}>${price}</span>
                  </button>
                );
              })}
            </div>
        ...
        {analysisPhase === "done" && analysisScore != null && (
          <hr style={{ border: "none", borderTop: "1px solid rgba(255,255,255,0.1)", margin: "0.75rem 0 0" }} />
        )}
        {analysisPhase === "done" && analysisScore != null && (
          <>
            <div className="demo-fade-in" style={{ display: "flex", justifyContent: "space-evenly", marginTop: "0.75rem" }}>
              {(["overview", "details"] as const).map((tab) => {
                const label = tab === "overview" ? null : "Original Details";
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      padding: "5px 16px",
                      borderRadius: "999px",
                      border: `1px solid ${active ? "rgba(57,255,157,0.4)" : "rgba(255,255,255,0.08)"}`,
                      background: active ? "rgba(57,255,157,0.07)" : "transparent",
                  ...
                  >
                    {tab === "overview"
                      ? <><span style={{ color: active ? "#3dff8f" : "#1a6e3f" }}>GENUINELY</span>{" Overview"}</>
                      : label}
                  </button>
                );
              })}
            ...
            {activeTab === "overview" && (
              <div className="ai-overview-dropdown demo-fade-in" style={{ borderTop: "none", paddingTop: 0, marginTop: "0.2rem" }}>
                <h3>Summary</h3>
                <p>
                  {SUMMARY_TEXT.slice(0, summaryChars)}
                  {summaryChars < SUMMARY_TEXT.length && <span className="demo-typing-cursor" />}
                </p>
                {summaryChars >= SUMMARY_TEXT.length && (
                  <button
                    type="button"
                    className="analyze-btn demo-fade-in"
                    style={{ marginTop: "1rem", fontSize: "0.78rem", opacity: 0.7, width: "100%", justifyContent: "center" }}
                    onClick={() => { setAnalysisScore(null); setCompressProgress(0); setFillValue(FILL_START_SCORE); setAnalysisPhase("loading"); }}
                    disabled={isAnalyzing}
                  >
            ...
            {activeTab === "details" && (
              <div className="ai-overview-dropdown demo-fade-in" style={{ borderTop: "none", paddingTop: 0, marginTop: "1rem", display: "flex", gap: 0, alignItems: "flex-start" }}>
                <dl style={{ width: "50%", display: "grid", gridTemplateColumns: "max-content 1fr", gap: "0.4rem 1.2rem", margin: 0, fontSize: "0.85rem", paddingRight: "2rem" }}>
                  {[
                    ...
                    <div key={k as string} style={{ display: "contents" }}>
                      <dt style={{ color: "rgba(255,255,255,0.4)", fontWeight: 600 }}>{k}</dt>
                      <dd style={{ margin: 0, color: "rgba(255,255,255,0.85)" }}>{v}</dd>
                    </div>
                  ) : null)}
                </dl>
                {demoListing.description && (
                  <div style={{ width: "50%", paddingLeft: "2rem", borderLeft: "1px solid rgba(255,255,255,0.1)" }}>
                    <p style={{ margin: "0 0 0.5rem", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.06em", color: "rgba(255,255,255,0.4)", textTransform: "uppercase" }}>Seller's Description</p>
                    <p style={{ margin: 0, fontSize: "0.85rem", color: "rgba(255,255,255,0.8)", lineHeight: 1.6 }}>{demoListing.description}</p>
                  </div>
                )}
              </div>
            )}
