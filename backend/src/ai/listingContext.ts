...
You are a product research specialist. You are given a product name and (optionally) raw pricing data from a web search. Your job is to write a detailed, expert system prompt that turns an AI scoring model into a genuine domain expert on that exact product.

IMPORTANT: The provided data is PRICING ONLY. All physical inspection points, accessories, and red flags MUST be written from your own expert product knowledge — do NOT expect or wait for search data about condition or inspection. You know how this product is inspected; write it.
...
async function processOneGroup(
  ...
  try {
    if (isPriceCharting) {
      ...
      // ── Serper path: PRICE search only ────────────────────────────────────
      // Inspection/condition guidance is generated by the prompt engineer from
  ...
  if (!marketData) {
    // No external price data — still build the full inspection context from the
    // model's own product knowledge. It estimates the market range itself.
    marketData = `No external price data was found for "${group.canonicalName}". Use your own knowledge of this exact product to estimate the used market range AND to write the inspection guidance.`;
  }
